...
class ConfigManager extends Component
{
    ...
    public function init()
    {
        ...
        $this->_base = empty($this->_base)
            ? require_once $this->basePath . '/config/web.php' : $this->_base;
    }
    ...
    {
        $this->basePath = \Yii::getAlias('@application'); //Yii::getPathOfAlias('application');
        ...
        $this->_cacheFilePath = \Yii::getAlias('@application'). DIRECTORY_SEPARATOR . $this->cacheFileName . '.php';
    }
    ...
    public function prepareSettings()
    {
                ...
                switch ($category) {
                    case 'modules':
                        if (!empty($moduleConfig['module'])) {
                            $settings['modules'] = ArrayHelper::merge(//CMap::mergeArray(
                    ...
                    default:
                        ...
                        if (!empty($moduleConfig[$category])) {
                            $settings[$category] = ArrayHelper::merge(//CMap::mergeArray(
        ...
        //debug_($settings);
       //debug_($this->mergeSettings($settings));

        return ArrayHelper::merge(// CMap::mergeArray(
    ...
    {
        //debug_($settings);
        $this->_config = ArrayHelper::merge(//CMap::mergeArray(
            $this->_base,
            [
...
//                ),
                // Модули:
                'modules'    => ArrayHelper::merge(//CMap::mergeArray(
                ...
                // Компоненты:
                'components' => ArrayHelper::merge( //CMap::mergeArray(
                ...
                // Алиасы
                'aliases' => ArrayHelper::merge(//CMap::mergeArray(
                ...
                // Алиасы
                /*'behaviors' => ArrayHelper::merge(//CMap::mergeArray(
        ...
        if (isset($this->_config['components']['urlManager']['rules'])) {
            ...
            $this->_config['components']['urlManager']['rules'] = ArrayHelper::merge(//CMap::mergeArray(
        ...
        if (isset($this->_config['components']['cache'])) {
            ...
            $this->_config['components']['cache'] = ArrayHelper::merge(//CMap::mergeArray(
    ...
    public function mergeRules($settings = [])
    {
        ...
        if (isset($settings['components']['urlManager'])) {
            ...
            $settings['components']['urlManager']['rules'] = ArrayHelper::merge( //CMap::mergeArray(
                $rules,
